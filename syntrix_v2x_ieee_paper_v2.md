# V2X: A Software-Defined Cloud-Synchronized Vehicle-to-Everything Emergency Clearance System with Real-World Dashboard Integration and C-V2X Resource Allocation Analysis
...
Abhishek Banapur^{1,a}, Ayush K Bhat^{1,a}, G Y Sagar^{1,a}, Harsha Patel T^{1,a}, Vishal^{1,a}
^{1}RV College of Engineering, Bengaluru, India, ^{a}Department of Computer Science and Engineering
...
4. A **detailed vehicle dashboard integration framework** covering Android Auto/CarPlay integration for modern vehicles and alternative interfaces (HUDs, haptics, acoustic beacons) for vehicles without digital displays.
...
The Wi-Fi-based family includes the mature IEEE 802.11p standard (operating in the 5.9 GHz ITS band with a 10-MHz bandwidth) and its successor IEEE 802.11bd, which introduces 256-QAM, MIMO, and midambles for channel estimation at speeds up to 500 km/h [5].

The Cellular-based family (C-V2X), standardized by the 3GPP, encompasses two generations:

1. **LTE-V2X (Release 14/15):** Utilizes the PC5 sidelink interface for direct V2V and V2I communication, based on device-to-device (D2D) protocols. It employs SC-FDMA at the MAC layer and turbo coding, with a fixed 15 kHz subcarrier spacing [6].

2. **5G-NR V2X (Release 16+):** Designed for advanced use cases (vehicle platooning, extended sensors, remote driving) requiring high reliability (> 99.999%) and low latency (< 3 ms). It introduces flexible numerologies (SCS of 15, 30, 60, and 120 kHz), LDPC coding, and a Physical Sidelink Feedback Channel (PSFCH) supporting unicast and groupcast modes.

### B. Previous Work on Emergency Vehicle Preemption

Noori et al. [1] demonstrated connected vehicle approaches but relied on DSRC hardware. Our work bridges these gaps by providing a complete, zero-hardware software stack with proven GPS filtering and multi-tier driver interface support.
...
**Table II: Mapping of V2X Architecture to C-V2X RA Modes**

| C-V2X Mode | Radio Mechanism | V2X Equivalent |
|---|---|---|
...
## IV. V2X System Design

### A. System Architecture

The V2X system consists of four browser-based client pages and a cloud backend, organized in three functional layers as shown in Fig. 1.

![Fig. 1. V2X System Architecture showing the three-layer design: Sensing Layer (GPS + Filters), Communication Layer (Firebase + OSRM), and Application Layer (V2V, V2I, Admin).](C:\Users\AYUSH K BHAT\.gemini\antigravity\brain\e4aa7ccf-c54d-4a0f-99e5-6329912e2dc4\system_architecture_1780490494938.png)
...
Raw GPS coordinates from mobile browsers are often inaccurate, particularly in high-density urban environments with multipath reflections from tall structures. The V2X system implements a three-tier sensor fusion pipeline to stabilize trajectories, as shown in Fig. 2.
...
Before filtering, the system computes the distance between the new GPS reading and the last known filtered state. If the implied velocity exceeds 200 m/s (720 km/h), the reading is rejected as a random jump caused by cell tower switching or GPS drift:

$$d_{jump} = haversine(x̂_{t-1}, z_t) \ \ \ If \ d_{jump} > 200m,\ reject\ z_t \ \ \ (1)$$
...
**Predict Phase:**
$$p_{t|t-1} = p_{t-1|t-1} + q \ \ \ (2)$$

**Update Phase:**
$$k_t = p_{t|t-1} / (p_{t|t-1} + r) \ \ \ (3)$$
$$x̂_{t|t} = x̂_{t|t-1} + k_t * (z_t - x̂_{t|t-1}) \ \ \ (4)$$
$$p_{t|t} = (1 - k_t) * p_{t|t-1} \ \ \ (5)$$
...
$$x_t^{(i)} = x_{t-1}^{(i)} + v^{(i)} * Δt + N(0, 10^{-5}) \ \ \ (6)$$

**Weight Update:** Particle weights are computed using a Gaussian likelihood relative to the measurement:
$$w_t^{(i)} = exp(-0.5 * (||x_t^{(i)} - z_t|| / σ)^2) \ \ \ (7)$$

where $σ = accuracy / 111320$ converts the GPS accuracy (meters) to degrees.
...
$$N_{eff} = 1 / Σ_{i=1}^{N} (w_t^{(i)})^2 \ \ \ (8)$$

### D. Dead Reckoning Fallback

If the GPS signal drops (e.g., in tunnels), the system extrapolates position for up to 5 seconds using current speed $v$ and bearing $θ$:

$$Δlat = (v * Δt * sin(θ)) / 111320 \ \ \ (9)$$
$$Δlng = (v * Δt * cos(θ)) / (111320 * cos(lat)) \ \ \ (10)$$
...
$$a = 6378137 m, \ \ \ b = 6356752.314245 m, \ \ \ f = 1 / 298.257223563$$

The step-by-step formula solves for the geodesic distance through reduced latitude transformations and convergence on the longitude difference $λ$. If the iterative solution does not converge (near-antipodal points), a Haversine fallback is used:

$$a_{hav} = sin^2(Δlat/2) + cos(lat_1) * cos(lat_2) * sin^2(Δlng/2) \ \ \ (11)$$
$$d = 2 R_e * arcsin(√(a_{hav})) \ \ \ (12)$$

### B. Yield Direction Calculation

The bearing from the EV to the civilian vehicle is computed as:
$$θ = atan2(sin(Δlng) * cos(lat_2), cos(lat_1) * sin(lat_2) - sin(lat_1) * cos(lat_2) * cos(Δlng)) \ \ \ (13)$$

The relative bearing determines the yield instruction:
$$θ_{rel} = (θ - ψ_{EV} + 360°) mod 360° \ \ \ (14)$$

- If $θ_{rel} < 180°$: Civilian is to the **right** → **"MOVE RIGHT"** advisory
- If $θ_{rel} \geq 180°$: Civilian is to the **left** → **"MOVE LEFT"** advisory
...
A big problem in developing countries is the high volume of older passenger cars, commercial trucks, auto-rickshaws, and two-wheelers that lack digital infotainment screens. We propose four alternative interaction paradigms:
...
Motorcycle riders face high ambient wind noise (> 80 dB at 60 km/h), making sound alerts useless. The V2X application connects via Bluetooth Low Energy to haptic actuators installed in:
...
This paper presented **V2X**, a software-defined, zero-hardware-cost Vehicle-to-Everything emergency clearance system. By using browser-based Geolocation APIs, a dual-stage Kalman + Particle filtering pipeline, and Firebase Realtime Database synchronization, the system achieves real-time V2V yield alerts (sub-500 ms latency) and V2I traffic signal preemption (1.4 s average transition time) without requiring any dedicated radio hardware.
...
[1] H. Noori, C. Olaverri-Monreal, and S. Selpi, "Connected Vehicle Approach for Emergency Vehicle Preemption," in *Proc. IEEE Int. Conf. Vehicular Electronics and Safety*, 2016, pp. 1–6.

[2] Google LLC, "Firebase Realtime Database Documentation," 2024. [Online]. Available: https://firebase.google.com/docs/database

[3] Leaflet Contributors, "Leaflet: An Open-Source JavaScript Library for Interactive Maps," 2024. [Online]. Available: https://leafletjs.com

[4] OSRM Project, "Open Source Routing Machine," 2024. [Online]. Available: https://project-osrm.org

[5] IEEE Standards Association, "IEEE Standard for Wireless Access in Vehicular Environments (WAVE)," IEEE Std 802.11p-2010, 2010.

[6] 3GPP, "Technical Specification Group Radio Access Network; Evolved Universal Terrestrial Radio Access (E-UTRA); Physical Layer Procedures (Release 14)," 3GPP TS 36.213, v14.7.0, 2018.

[7] National Crime Records Bureau, "Accidental Deaths & Suicides in India (ADSI) — 2022," Ministry of Home Affairs, Govt. of India, 2023.

[8] 3GPP, "NR Sidelink Relay; Stage 2 (Release 17)," 3GPP TS 23.304, v17.4.0, 2023.

[9] 5GAA, "C-V2X Use Cases and Service Level Requirements: Volume II," White Paper, 5G Automotive Association, 2020.

[10] M. Gonzalez-Martin, M. Sepulcre, R. Molina-Masegosa, and J. Gozalvez, "Analytical Models of the Performance of C-V2X Mode 4 Vehicular Communications," *IEEE Trans. Vehicular Technology*, vol. 68, no. 2, pp. 1155–1166, Feb. 2019.

[11] W3C, "Geolocation API Specification," W3C Recommendation, 2022. [Online]. Available: https://www.w3.org/TR/geolocation/

[12] Android Developers, "Build Apps for Android Auto," 2024. [Online]. Available: https://developer.android.com/training/cars

[13] Apple Inc., "CarPlay App Programming Guide," 2024. [Online]. Available: https://developer.apple.com/carplay/

[14] SAE International, "Taxonomy and Definitions for Terms Related to Cooperative Driving Automation for On-Road Motor Vehicles," SAE J3216, 2022.
